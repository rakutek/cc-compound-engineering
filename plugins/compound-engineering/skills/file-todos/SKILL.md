---
name: file-todos
description: このスキルは、todos/ ディレクトリのファイルベースの todo 追跡システムを管理する際に使用されるべきです。todo の作成、ステータスと依存関係の管理、トリアージの実施、スラッシュコマンドやコードレビュープロセスとの統合のワークフローを提供します。
---
...
| PR comments | `/resolve` → Individual fixes → Todos | gh CLI + skill |
| Code TODOs | `/resolve` → Fixes + Complex todos | Agent + skill |
